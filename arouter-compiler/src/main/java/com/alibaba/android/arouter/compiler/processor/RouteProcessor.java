...
public class RouteProcessor extends BaseProcessor {
    ...
    private void parseRoutes(Set<? extends Element> routeElements) throws IOException {
        if (CollectionUtils.isNotEmpty(routeElements)) {
            ...
            // Interface of ARouter  root表和group表的父类型
            TypeElement type_IRouteGroup = elementUtils.getTypeElement(IROUTE_GROUP);
            TypeElement type_IProviderGroup = elementUtils.getTypeElement(IPROVIDER_GROUP);
            //group的value 对应RouteMeta， RouteType表示这个value存放的类型，fragment 还是activity？
            ...
            for (Element element : routeElements) {
                ...
                if (types.isSubtype(tm, type_Activity) || types.isSubtype(tm, fragmentTm) || types.isSubtype(tm, fragmentTmV4)) {
                    ...
                    for (Element field : element.getEnclosedElements()) {
                        if (field.getKind().isField() && field.getAnnotation(Autowired.class) != null && !types.isSubtype(field.asType(), iProvider)) {
                            ...
                            paramsType.put(injectName, typeUtils.typeExchange(field)); //TODO
                ...
                categories(routeMeta); //TODO for 循环完成 这里就已经把把GroupMap 都分好组了
            }
            ...
            for (Map.Entry<String, Set<RouteMeta>> entry : groupMap.entrySet()) {
                ...
                for (RouteMeta routeMeta : groupData) {
                    RouteDoc routeDoc = extractDocInfo(routeMeta); //TODO doc文档造作
                    ...
                    switch (routeMeta.getType()) {
                        case PROVIDER:  // Need cache provider's super class
                            ...
                            for (TypeMirror tm : interfaces) {
                                ...
                                if (types.isSameType(tm, iProvider)) {   //TODO 直接 集成Iprovider
                                    ...
                                    loadIntoMethodOfProviderBuilder.addStatement(
                                            ...
                                            routeMeta.getPath(),
                                            routeMeta.getGroup());
                                } else if (types.isSubtype(tm, iProvider)) { //TODO double interface 集成Iprovider
                                    // This interface extend the IProvider, so it can be used for mark provider
                                    loadIntoMethodOfProviderBuilder.addStatement(
                                            "providers.put($S, $T.build($T." + routeMeta.getType() + ", $T.class, $S, $S, null, " + routeMeta.getPriority() + ", " + routeMeta.getExtra() + "))",
                    ...
                    if (MapUtils.isNotEmpty(paramsType)) {
                        ...
                        for (Map.Entry<String, Integer> types : paramsType.entrySet()) {
                            ...
                            RouteDoc.Param param = new RouteDoc.Param(); //TODO doc RouteDoc.Param
                ...
                rootMap.put(groupName, groupFileName); //TODO root 表
            ...
            JavaFile.builder(PACKAGE_OF_GENERATE_FILE,  //TODO 写入provider
            ...
            String rootFileName = NAME_OF_ROOT + SEPARATOR + moduleName;  //TODO 写入Root表
            JavaFile.builder(PACKAGE_OF_GENERATE_FILE,
